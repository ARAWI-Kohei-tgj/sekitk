...
if(Shape !is MatrixType.zero &&
  ...
  static if(Shape is MatrixType.dense || Shape is MatrixType.upperTri){
	...
	static if(Shape is MatrixType.upperTri){
	  foreach(scope i; 1u..LEN) num[i] -= sumFromZero(i);
	}
      }
      else static if(MatOdr is MajorOrder.column){
	static if(Shape is MatrixType.dense){
	  num= recurrence!( (a, n) => a[n-1]+Size )(TypeOfIndex(Size)).staticArray!LEN;
	}
	else{
...
if(Shape !is MatrixType.zero &&
  ...
  static if(Shape is MatrixType.dense || Shape is MatrixType.lowerTri){
	...
	num[0]= 1;
	static foreach(scope i; 1u..num.length){
	...
	static if(Shape is MatrixType.dense){
	  num[0]= Size;
	  static foreach(scope TypeOfIndex i; 1..num.length) num[i]= cast(TypeOfIndex)(num[i-1]+Size);
	}
	else{
	  static foreach(scope TypeOfIndex i; 0..num.length) num[i]= sumFromZero(cast(TypeOfIndex)(i+1u));
	}
      }
      else{
	num[0]= Size;
	static foreach(scope TypeOfIndex i; 1..num.length) num[i]= num[i-1]+2*(Size-i);
      }
      return num;
    }();
  }
...
if((Size > 0 && Typ is IndexSetType.diag) ||
  ...
  @safe pure nothrow @nogc const{
    ...
    immutable(TypeOfIndex) totalStep() @property{
      TypeOfIndex num= 0u;
      foreach(scope slc; 0..MAX_ITR_SLICE) num += MAX_ITR_LOCAL[slc];
      return num;
    }
    ...
    auto itrPosition(in size_t totalStep){
      ...
      foreach(scope TypeOfSize slc; 0..MAX_ITR_SLICE){
	numBef= numCurr;
    ...
    TypeOfSize totalStepCurr() @property{
      ...
      foreach(scope idx; 0u..stepSlice) result += MAX_ITR_LOCAL[idx];
      result += stepLocal;
