...
public:
  ...
  @safe pure nothrow const{
	...
	auto idxset= IndexSetStrictTriL!(Size, ShapeDest, MatOdrDest)();
      ...
      foreach(scope i; idxset) num[i]= VALUE_ZERO;

      {
	auto idxset= IndexSetDiag!(Row, Column, ShapeDest, MatOdrDest)();
	size_t j= 0u;
	foreach(scope i; idxset) num[i]= this._values[j++];
      }
      return new typeof(return)(num);
    }
  }
...
private:
  ...
  @safe pure nothrow const{
    ...
    T[arrayLength!(Size, Size, MatrixType.dense)] arrayDense() @nogc{
      ...
      foreach(scope TypeOfIndex i; idxSet) temp[i]= this._values[j++];
      return temp;
    }
    ...
    T detImpl() @nogc{
      ...
      foreach(scope num; _values[1u..$]) result *= num;
      return result;
    }
