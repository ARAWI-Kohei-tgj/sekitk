	...
	@safe pure nothrow const{
		...
		if(isPlusOrMinusSign!PM && (ShapeR is MatrixType.dense ||
			...
			static if(ShapeR is MatrixType.dense){
				foreach(idx; 0u..LEN) mixin("num[idx]" ~Op ~"rhs.v[idx];");
			}
			else{
				auto idxSetDest0= IndexSetDiag!(Row, Column, ShapeDest, MatOdr)();
...
private:
	@safe pure nothrow @nogc const{
		...
		T[arrayLength!(Size, Size, MatrixType.dense)] arrayDense(){
			...
			foreach(TypeOfIndex i; merge(idxSet0, idxSet1)) num[i]= this._values[j++];
		  ...
		  immutable TypeOfIndex[Size] idxSetA= (){
			...
			const T[Size+2] phi= (){
			  ...
			  temp[Size-1]= this._values[idxSetA[$-1u]];
				foreach_reverse(i; 1u..Size-1u) temp[i]= this._values[idxSetA[i]]*temp[i+1];
				return temp;
			}();
			...
			foreach(i; 0..Size){
				foreach(j; 0..Size){
					if(i <= j){
						...
						foreach(k; i..j-1u) num[idxMap(i, j)] *= this._values[idxSetB[k]];
					} 
					else{
						num[idxMap(i, j)]= theta[j-1]*phi[i+1];
						foreach(k; j..i-1u) num[idxMap(i, j)] *= this._values[idxSetC[k]];
					}
