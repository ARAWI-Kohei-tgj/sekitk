...
enum string UPPER_TRI= q{
  ...
  @safe pure nothrow const{
    ...
    if(isPlusOrMinusSign!Op){
      ...
      num= this._values[];
      foreach(scope TypeOfIndex i; idxSet) mixin("num[i]" ~Op ~"= rhs._values[j++];");
      ...
      {
	{
	  ...
	  foreach(scope idx; idxSet) num[idx]= VALUE_ZERO;
	}
	{
	  TypeOfIndex j= 0u;
	...
	foreach(scope numL, scope numR; zip(elmLhs, elmRhs)) num[k++]= numL+numR;
      }
	...
	idxLhs= Column-1u;
	foreach(scope i; iota!TypeOfSize(Row)){
	  foreach(scope j; iota!TypeOfSize(ColumnR)){
	    ...
	    foreach(scope k; 1u..Column-i) num[idxPrd] += this._values[idxLhs-k]*rhs._values[idxRhs-k*ColumnR];
	  }
	  idxLhs += Column-(i+1u);
	}
	break;
	...
	foreach(scope i; 0u..Row){
	  foreach(scope j; 0u..ColumnR-i){
	    idxPrd= idxsetLhs[j]+i;
	    idxLhs= idxsetLhs[j];
	    num[idxPrd]= this._values[idxLhs]*rhs._values[idxLhs+i];
	    foreach(scope k; 1u..i+1u) num[idxPrd] += this._values[idxLhs+k]*rhs._values[idxsetRhs[j+k]+(i-1u)-(k-1u)];
	  }
	}
	break;
...
package:
  @safe pure nothrow @nogc const{
	...
	case MajorOrder.column:
      ...
      foreach(scope idxDest, scope idxInternal;
