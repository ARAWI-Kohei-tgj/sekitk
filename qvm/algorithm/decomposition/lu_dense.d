...
enum string LU_DENSE_IMPL2= q{
	...
	if(isNormalLU!Method && (is(ArgType: T[arrayLength!(Size, Size, Shape)]) ||
		...
		foreach(TypeOfSize idxD; 0u..Size){	// row major order -> seeks horizontally
			mixin(PIV_IMPL);	// pivotting

			static if(isDoolittle!Method){
				foreach(i; idxD+1u..Size){	// row major order -> seek vertically
					a[i][idxD] /= a[idxD][idxD];
					foreach(k; idxD+1u..Size) a[i][k] -= a[i][idxD]*a[idxD][k];
				}
			}
			else static if(isCrout!Method){
				foreach(i; idxD+1u..Size){
					a[idxD][i] /= a[idxD][idxD];
					foreach(k; idxD+1u..Size) a[k][i] -= a[k][idxD]*a[idxD][i];
				}
			}
			else{
				static assert(false);
				...
				if(_invertible){
					...
					foreach(TypeOfSize j; 0u..Size){
						foreach(TypeOfSize i; 0u..Size){
							...
							foreach(TypeOfSize k; 0u..i) inv2d[i][j] -= _values[indexMap!(Size, Size, Shape, MatOdr)(i, k)]*inv2d[k][j];
						}

						foreach_reverse(i; 0u..Size){	// it must be an down counting
							foreach(k; i+1u..Size) inv2d[i][j] -= _values[indexMap!(Size, Size, Shape, MatOdr)(i, k)]*inv2d[k][j];
			...
			matrix(DecomposedMat Mat: DecomposedMat.lowerTri)() nothrow @nogc{
				...
				static if(isDoolittle!Method){
					{	// diagonal elements
						...
						foreach(TypeOfIndex idx; idxSetDest) result[idx]= VALUE_ONE;
					}
					{	// strict lower triangular elements
						...
						foreach(idxSrc, idxDest; zip(idxSetSrc, idxSetDest)) result[idxDest]= _values[idxSrc];
					}
				}
				else{
