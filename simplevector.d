...
if((isFloatingPoint!T || isComplex!T || isSignedInt!T) && Size > 0u){
  ...
  @safe pure nothrow @nogc const{
    ...
    Vec opUnary(string Op: "-")(){
      T[Size] buf;
      foreach(scope idx; 0u..Size) buf[idx]= -this.num[idx];
      return Vec(buf);
    }
    ...
    BaseType opBinary(string Op: "*")(in typeof(this) rhs){
      ...
      foreach(scope idx; 0u..Size) result += num[idx]*rhs.num[idx].conj;
  ...
  @safe const{
    ...
    if(isOutputRange!(Writer, const(Char)[])){
      ...
      w.put(PAREN_ST);
      foreach(scope size_t idx, scope T elm; this.num){
